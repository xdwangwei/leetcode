...
public class _32_LongestValidParentheses {
     ...
     * )()())           0出栈，栈空，5入栈
     * <p>
     * 每次遇到(下标入栈，)就弹出，其实栈中留着的下标相当于是一个标记，从开始到当前位置的有效长度已统计过
     * 如刚开始相当于从开始到-1位置的串已统计过，每次()()成对出现时，不用更新res，因为成对出现的有效串总会被弹完
     * 而每次弹完栈不空，说明上上一个位置到现在这个位置又是一个有效串，所以可以更新res
     *
     * @param s
     */
    public int solution3(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (s.charAt(i) == '(') stack.push(i);
                ...
                stack.pop();
                // 栈空了，把当前下标压栈，相当于从开始截止到当前位置是一个有效串
                if (stack.isEmpty()) {
                    stack.push(i);
                } else {
                    // stack.peek()获取栈顶元素，但不移除
                    // 栈顶始终保持着之前那个有效串的末尾字符下表
                    // 栈不空说明从上一次截止到这个位置，出出进进了多个()()，这部分的有效长度就是当前下表-上次的末尾位置
